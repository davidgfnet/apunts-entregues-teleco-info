...
/*	Justificación:
	La función es correcta y se sigue cumpliendo el invariante al finalizar. En caso de existir la cuenta no se añade de nuevo, de forma
	que no aparecerán dos cuentas con la misma "id_banco" y el mismo "banco". En caso de no existir la cuenta se crea una y se enlaza con la tabla 
	de hash de manera que TablaCuentas[i] apunta a la nueva cuenta (donde i es el hash de cuenta). Las demás cuentas siguen siendo accesibles 
	a través de TablaCuentas[i]->siguiente. Dado que la estructura anterior era válida, la nueva lo será, ya que es la misma más un elemento 
	enlazado con la tabla y la anterior estructura.
	Si el titular ya está entre los apuntados por cuenta->L_titulares o sus respectivos "siguiente", no lo añadimos; de ésta forma no apareceran
	dos titulares con el mismo NIF apuntando a la misma cuenta. En caso contrario añadimos un titular que es apuntado por cuenta->L_titulares. 
	En caso de haber más titulares éstos pasaran a ser apuntados por cuenta->L_titulares->siguiente.
	todos los titulares son accesibles.
	...
	Podemos observar que los cambios sobre la estructura han sido aplicados de forma que la estructura resultante siga
	cumpliendo con los requisitos impuestos por el invariante. 
...
/*	Justificación:
	...
	Observamos que pese a los cambios aplicados sobre la estructura, la estructura resultate sigue cumpliendo con los requisitos
	impuestos por el invariante.
*/
...
/*	JUSTIFICACIÓN:
	el invariante garantiza que en TablaCuentas enlazados mediante 'siguiente' 
	...
	Una vez tengamos la cuenta, el invariante nos garantiza que L_titulares apunta a la lista de titulares de esa cuenta, la recorreremos e 
	imprimiremos todos los titulares.
*/
...
/*	JUSTIFICACIÓN:
	El invariante garantiza que en TablaTitulares enlazados mediante 'mismo_hash' 
	...
	de las que p es titular (ya que como indica el invariante p puede aparecer varias veces con diferentes cuentas).
*/
...
/*
 ...
 ---
 	...
 	Se ha elegido una implementación de la estructura mediante dos tablas de hash con encadenamiento. De este modo conseguimos que 
 ...
 ---
 	...
 	apunta a un titular de dicha cuenta.
 	Los titulares, a su vez, cuentan con un campo 'siguiente' que apunta a otro titular de esa misma cuenta de modo que siguiendo dicho
 	enlace podemos acceder a todos los titulares de la cuenta en cuestión.
 	Ningún nodo es apuntado por más de un enlace "siguiente". No hay dos cuentas en la estructura en que coincidan los campos "banco" e 
	"id_cuenta"

 	Para toda 0 <= i < 23 se cumple que TablaTitulares[i] apunta a NULL o a bien a un titular t tal que la imagen de hash de t->NIF = i.
 	Cada t apunta a su vez mediante el enlace 'mismo_hash' a otro titular con i como imagen de hash de modo que siguiendo dicho enlace
 	podemos acceder a todos los titulares cuya imagen de hash sea i.
 	...
 	embargo no apuntanan a la misma cuenta.
 	...
 	El invariante siempre se cumple:
 		...
 		-> si el invariante se cumple al iniciarse la inclusión, al terminar ésta y atendiendo a los argumentos expuestos en la justificación
 		   de dicha función, el invariante se sigue cumpliendo.
        -> si el invariante se cumple al iniciarse la eliminación, al terminar ésta y atendiendo a los argumentos expuestos en la justificación
           de dicha función, el invariante se sigue cumpliendo.
 		-> ninguna de las operaciones de consulta modifica Bancario, de modo que si el invariante se cumplía al inicio, seguirá
		 cumpliéndose una vez realizada la consulta.	
 ---
 
 		-> Equivalencia <-
 		
 	Dos estructuras Bancario B1 y B2 son equivalente si para todo 0 <= i < 23 se tiene que:
	  -	desde B1.TablaTitulares[i]  y   B2.TablaTitulares[i] y siguiendo el enlace "mismo_hash" hemos de ser capaces de acceder a los 
		mismos titulares (deben coincidir los campos NIF y cuenta), aunque tal vez en diferente orden.
          -	desde B1.TablaCuentas[i]  y B2.TablaCuentas[i] y siguiendo el enlace "siguiente" hemos de ser capaces de acceder a las mismas
		cuentas (deben coincidir los campos "banco" y "id_cuenta"), aunque tal vez en diferente orden.
	En dos estructuras Bancario equivalentes las dos operaciones de consulta imprimen la misma información, aunque quizás
	en diferente orden una de la otra.
