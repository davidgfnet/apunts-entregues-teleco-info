...
void list_add_right_tail(ptype list, ptype itemtoadd) {
	if (list->down == 0) {
		list->down = itemtoadd;
	}else{
		ptype tailitem = list->down;
		while (tailitem->right != 0)
			tailitem = tailitem->right;

		tailitem->right = itemtoadd;
	}

	itemtoadd->right = 0; // Make sure null-terminates!
}
...
bool equivalent_types(ptype t1,ptype t2) {
  if (t1->kind == "array" and t2->kind == "array") {
    return (t1->numelemsarray == t2->numelemsarray and 
            equivalent_types(t1->down,t2->down));
  }else if (t1->kind == "struct" and t2->kind == "struct") {
    return false;
  }else{
