...
void Model::Render(float material[13], bool custom_color, ColorI col)
{
  if (custom_color) {
  ...
  for(unsigned int cara = 0; cara < faces.size(); cara++) {
    ...
    if (not custom_color) {
      ...
      glMaterialfv(GL_FRONT,GL_SHININESS,&material[12]);
    }
...
void Model::readObj(const char* filename, MaterialLib& matlib)
{
  ...
  if (!fp)
  {
	...
	float x = readfloat (words[1]);
	float y = readfloat (words[2]);
	float z = readfloat (words[3]);
	
	if (nwords == 5) 
	{
	  float w = readfloat (words[4]);
	  x/=w;
	  y/=w;
	  z/=w;
	}
