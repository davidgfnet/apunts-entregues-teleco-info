...
void MaterialWidget::MakeInvisible(float * mat) {
    this->setVisible(false);

    if (mat != NULL) {
      memcpy(mat,old_materials,13*sizeof(float));
    }
}
...
void MaterialWidget::MakeVisible(float ar, float ag, float ab,
    ...
    materials[12] = s;
    memcpy(old_materials, materials,sizeof(materials)); // Copy!
...
void MaterialWidget::Revert() {
  memcpy(materials, old_materials,sizeof(materials)); // Copy!
  emit(MaterialChange(materials));
}
