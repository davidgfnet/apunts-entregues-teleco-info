...
private:
	...
	S Max_int(node *n) {
		if (n == NULL) return S();
		if (n->r == NULL) return n->key;
		else return Max_int(n->r);
	}
...
int main () {
	...
	while (cin >> p) {
		...
		if (not esta) {
			...
			int * c = vec.getElemgetElem (p);
			*c = (*c)+1;
		}
	}
	string maxima = vec.GetMax();
	int * c = vec.getElemgetElem (maxima);
	cout << maxima << " " << *c << endl;
}
